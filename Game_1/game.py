...
#
#    norm_input = json.dumps(my_dict), separators = " "
#    elif text.isdigit() == True:
#        print("Please enter a valid sentance.")

#Attempt 3
#    text_list = collections.defaultdict(list)
#    text = text.strip(',.?!-_;:[]@£$%^&&*()`¬¦/\|#~=+0123456789')
#    words = text.split()
#    for k, v in zip(words[:-1], words[1:]):
#        text_list[k].append(v)
#        text_list.join(" ")
#        print(str(text_list))
...
# Attempt 4
    ...
    norm_txt = ''.join(char for char in text if char not in exclude)
    return norm_txt
...
def normalise_input(user_input):
    ...
    normalised_txt_1 = ''
    normalised_txt_2 = remove_punct(user_input)
    normalised_txt_1 = remove_spaces(normalised_txt_2)
    normalised_txt_1 = normalised_txt_1.strip('0')
    normalised_txt_1 = normalised_txt_1.strip('1')
    normalised_txt_1 = normalised_txt_1.strip('2')
    normalised_txt_1 = normalised_txt_1.strip('3')
    normalised_txt_1 = normalised_txt_1.strip('4')
    normalised_txt_1 = normalised_txt_1.strip('5')
    normalised_txt_1 = normalised_txt_1.strip('6')
    normalised_txt_1 = normalised_txt_1.strip('7')
    normalised_txt_1 = normalised_txt_1.strip('8')
    normalised_txt_1 = normalised_txt_1.strip('9')
    normalised_txt_1 = normalised_txt_1.replace('go ', '')
    normalised_txt_1 = normalised_txt_1.lower()
    return normalised_txt_1
...
def menu(exits):
    ...
    while True:
        ...
        if is_valid_exit(exits, norm_user_input) == True:
            return norm_user_input
        else:
            user_input = input("Please try another exit: ")
